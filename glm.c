...
======================================================================
      ...
      Modified by Andrei Borovsky, 2002
      borovsky@pochtamt.ru
      Modifications:
      GLM_WIREFRAME mode added.
  ...
  for (i = 0; i < nummaterials; i++)
    {
      ...
      model->materials[i].texture = NULL;
      model->materials[i].texture_wid = 0;
      model->materials[i].texture_hgt = 0;
    }
  ...
  while (fscanf (file, "%s", buf) != EOF)
	...
	case 'm':
	  ...
	  if (strcmp (buf, "map_Kd") == 0)
		{
	  	  fscanf (file, "%s", buf);
		  model->materials[nummaterials].texture = glmReadPPM (buf, &model->materials[nummaterials].texture_wid, &model->materials[nummaterials].texture_hgt);
		  /* eat up rest of line */
	      fgets (buf, sizeof (buf), file);
		}
	  break;
...
static GLvoid
_glmSetDefaultMaterial (GLMmodel * model)
{
  ...
  model->materials[0].name = strdup ("default");
  model->materials[0].texture = NULL;
  model->materials[0].texture_wid = 0;
  model->materials[0].texture_hgt = 0;
}
...
glmSecondPass (GLMmodel * model, FILE * file)
{
  ...
  printf (" Memory: %d bytes\n",
	  ...
	  numnormals * 3 * sizeof (GLfloat) * (numnormals ? 1 : 0) + numtexcoords * 3 * sizeof (GLfloat) * (numtexcoords ? 1 : 0) + numtriangles * sizeof (GLMtriangle));
#endif
}
...
/* !FIXME: this surely should be freed, but it fails:
free(): invalid pointer 0x94a7080!
 ...
 *             GLM_TEXTURE2 -  draw the texture map
...
glmWriteOBJ (GLMmodel * model, char *filename, GLuint mode)
{
  ...
  if (mode & GLM_FLAT && mode & GLM_SMOOTH)
    {
      printf ("glmWriteOBJ() warning: flat normal output requested " "and smooth normal output requested (using smooth).\n");
      mode &= ~GLM_FLAT;
    }
  if (mode & GLM_COLOR && !model->materials)
    {
      printf ("glmWriteOBJ() warning: color output requested " "with no colors (materials) defined.\n");
      mode &= ~GLM_COLOR;
    }
  if (mode & GLM_MATERIAL && !model->materials)
    {
      printf ("glmWriteOBJ() warning: material output requested " "with no materials defined.\n");
      mode &= ~GLM_MATERIAL;
    }
  if (mode & GLM_COLOR && mode & GLM_MATERIAL)
    {
      printf ("glmWriteOBJ() warning: color and material output requested " "outputting only materials.\n");
      mode &= ~GLM_COLOR;
    }
	  ...
	  if (mode & GLM_SMOOTH && mode & GLM_TEXTURE)
	    {
	      fprintf (file, "f %d/%d/%d %d/%d/%d %d/%d/%d\n",
		       ...
		       T (group->triangles[i]).tindices[1], T (group->triangles[i]).vindices[2], T (group->triangles[i]).nindices[2], T (group->triangles[i]).tindices[2]);
	    }
	  else if (mode & GLM_FLAT && mode & GLM_TEXTURE)
	    {
	      fprintf (file, "f %d/%d %d/%d %d/%d\n",
		       T (group->triangles[i]).vindices[0],
		       T (group->triangles[i]).findex, T (group->triangles[i]).vindices[1], T (group->triangles[i]).findex, T (group->triangles[i]).vindices[2], T (group->triangles[i]).findex);
	    }
	  else if (mode & GLM_TEXTURE)
	    {
	      fprintf (file, "f %d/%d %d/%d %d/%d\n",
		       T (group->triangles[i]).vindices[0],
		       T (group->triangles[i]).tindices[0],
		       T (group->triangles[i]).vindices[1], T (group->triangles[i]).tindices[1], T (group->triangles[i]).vindices[2], T (group->triangles[i]).tindices[2]);
	    }
	  else if (mode & GLM_SMOOTH)
	    {
	      fprintf (file, "f %d//%d %d//%d %d//%d\n",
		       T (group->triangles[i]).vindices[0],
		       T (group->triangles[i]).nindices[0],
		       T (group->triangles[i]).vindices[1], T (group->triangles[i]).nindices[1], T (group->triangles[i]).vindices[2], T (group->triangles[i]).nindices[2]);
	    }
	  else if (mode & GLM_FLAT)
	    {
	      fprintf (file, "f %d//%d %d//%d %d//%d\n",
 ...
 *             GLM_COLOR    -  render with colors (color material)
 *             GLM_MATERIAL -  render with materials
 *             GLM_WIREFRAME - render wireframe model
 *             GLM_COLOR and GLM_MATERIAL should not both be specified.
 *             GLM_FLAT and GLM_SMOOTH should not both be specified.
 */
GLvoid
  ...
  while (group)
    {
      ...
      if (mode & GLM_MATERIAL || mode & GLM_COLOR || mode & GLM_TEXTURE2)
	{
...
/*	  material = &model->materials[group->material]; */
      ...
      if (mode & GLM_TEXTURE && mode & GLM_TEXTURE2 && material->texture != NULL)
	{
#ifdef GL_VERSION_1_1
	/* In real program, texture names should be generated by glGenTextures() rather than by using material index */
      	if (!glIsTexture(group->material+1))
	{
		glBindTexture (GL_TEXTURE_2D, group->material+1);
#endif
		...
		glTexImage2D (GL_TEXTURE_2D, 0, GL_RGB, material->texture_wid, material->texture_hgt, 0, GL_RGB, GL_UNSIGNED_BYTE, material->texture);
#ifdef GL_VERSION_1_1
	} else {
		glEnable (GL_TEXTURE_2D);
		glBindTexture (GL_TEXTURE_2D, group->material+1);
